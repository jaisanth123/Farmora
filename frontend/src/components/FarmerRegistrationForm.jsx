...
const FarmerRegistrationForm = () => {
    ...
    const checkUserRegistration = async () => {
      if (userId) {
        try {
          ...
          const response = await axios.get(`http://localhost:5000/api/farmers/profile/${userId}`);
  ...
  const getLocationData = async () => {
    try {
      ...
      const coordResponse = await axios.get(
        `http://localhost:8000/api/coordinates?place=${encodeURIComponent(placeQuery)}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    if (!farmerInfo.name || !farmerInfo.age || !farmerInfo.state || !farmerInfo.district) {
    ...
    console.log("Final farmerData before sending:", JSON.stringify(farmerData, null, 2));
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-md w-full mx-auto space-y-8">
        <div>
          <h1 className="text-3xl font-bold text-center text-gray-900">Complete Your Profile</h1>
